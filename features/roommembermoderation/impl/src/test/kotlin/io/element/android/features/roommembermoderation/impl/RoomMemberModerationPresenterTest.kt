...
class RoomMemberModerationPresenterTest {
    ...
    fun `present - confirming a role change applies it once`() = runTest {
        ...
        createRoomMemberModerationPresenter(room = room).test {
            val initialState = awaitState()
            initialState.eventSink(
                RoomMemberModerationEvents.ProcessAction(ModerationAction.ChangeRole(RoomMember.Role.Moderator), targetUser)
            )
            advanceUntilIdle()
            ...
            initialState.eventSink(InternalRoomMemberModerationEvents.DoChangeRole)
            advanceUntilIdle()
